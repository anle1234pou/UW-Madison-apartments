	...
	$("input[type='radio']").click(function(){
		...
		gg2 = L.geoJson(data, {
			...
			onEachFeature(feature, layer) {
				...
				createInfoPopup(feature, layer, stylePass)
			}
		}).addTo(map)
	});
...
function createInfoPopup(feature,layer,style) {
...
function ajax10() {
	return $.ajax("data/10_null_min_walk_polygon+reachable_restaurant_id.geojson", {
		/* specify the file format the ajax method is to call*/
		dataType: "json",
		success: function(response) {
			//var attributes = processData(response)
