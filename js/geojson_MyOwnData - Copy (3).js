...
function all_ress_5(map, data_res, ress5) {
	...
	gg5 = L.geoJson(data_res, {
		...
		onEachFeature(feature, layer) {
			...
			layer.bindTooltip(toolTipContent,{'permanent':true, 'direction':'center'}).openTooltip()
	...
	var panelContent = "<div id='ress_info'><p>Number of restaurants within 5-min walk distance: <b>" + ress5.length + '<b></p></div>'
...
function resSymbols(data, data5, data10, data_res, map) {
	...
	gg0 = L.geoJson(data, {
		...
		onEachFeature(feature, layer) {
			layer.on({
				mouseover: function() {
					layer.setStyle({color:"#99bb11",weight:3})
					
				},
				mouseout: function() {
					layer.setStyle({"color": "#999999", "fillColor": "#cccccc","fillOpacity": 1,"weight": 1})
				},
				click: function() {
