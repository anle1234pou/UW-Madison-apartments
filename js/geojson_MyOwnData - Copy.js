...
function createInfoPopup(feature,layer) {
	...
	$(layer).on({
		...
		mouseout: function() {
			...
			layer.closePopup()
			if (typeH == 'House') {
				layer.setStyle({color:'#ddcc00',fillColor:"#cccc00"})
			} else {
				layer.setStyle({color:'#ff55ff',fillColor:"#ff55ff"})
			}
		},
		click: function() {
			layer.openPopup()
			console.log('passing: ', addr)
			updateInfoWindow(addr,name,typeH,url1,url2,det,yrB,flp)
			if (typeH == 'House') {
...
function updateInfoWindow(addr,name,typeH,url1,url2,det,yrB,flp) {
	$('#updatable').remove()
	$('#tb_flp').remove()

	var panelCntnt = '<div id="updatable"><p id="updatable-basic"><b>Name: </b>' + name + '</p>'
...
function createRepSymbols(data, map) {
	...
	L.geoJson(data, {
		...
		onEachFeature(feature, layer) {
			console.log(feature)
			updateColor(feature, layer)
			createInfoPopup(feature, layer)
		}
		/*, {
...
function enableMenuButtons(map) {
	// append a series of buttons to '#panel', '#compare' and '#compareResult' interfaces
	$('#menu').append("<p id='title_menu'>UW-Madison Off-Campus Housing Options Explore Tool</p>")
	...
	$('#menu').append("<br><button id='restaurant_proximity'>To Restaurants</button>")
